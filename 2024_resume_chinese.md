...
1. 開發出圖數據產品原型，並實際導入有客戶風險和商機探索需求之相關業務部門。

**責任**

1. 與專案經理一起進行使用者訪談，並將見解轉化為產品開發藍圖。

2. 帶領3名資料科學家進行原資料探索和轉換，以豐富資料庫之內容與產品應用場景。

**成果**

1. 從頭構建出兩個客戶洞察應用程式的核心數據庫，內容包含15種的關係連線和7種類型的實體（人員/公司/帳戶/IP/裝置/電話/電子郵件）。

2. 建置出Neo4j開發環境，以加速新戶關聯查詢和警示帳戶樣態發掘。

3. 從頭構建出RDB到GraphDB的ETL架構，以確保產品的可擴充和可維護性。
...
2. 透過可定製的資料驗證、元資料註冊和執行時間/記憶用量分析工具，降低資料處理流程之維護、維護與執行成本。

3. 確保Python相關表格處理套件（如CuDF、Polars、PySpark等）的可兼容性，以實現未來資料流的GPU加速，目標是加速1000倍。
...
- 將框架包裝成一個可行的python軟體包，並將其成功導入到兩個營運專案中
