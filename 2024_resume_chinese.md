...
2. **程式重構和遷移：**透過徹底理解業務邏輯並將其重構為Chain of command設計模式，提升業務邏輯的可讀性。 設計帶有日誌記錄機制的通用模組，以降低未來的維護成本。 規劃並在不損害7x24標準的情況下成功完成遷移。

3. **重新設計出可解釋的房價評估模型：**與研究機構和業務部門合作，建立可解釋的深度學習模型。 重新設計損失函式，以滿足法律要求。 採用谷歌最先進的最近鄰居演算法進行參考房屋搜尋，縮短模型推理時間。
